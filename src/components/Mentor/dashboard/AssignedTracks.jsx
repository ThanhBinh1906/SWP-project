...
export function AssignedTracks({ teams = [] }) {
  ...
  return (
    <MentorPanel title="Track được phân công" subtitle="Các team được gom theo track từ dữ liệu thực tế" icon={mentorIcons.GitBranch}>
        ...
        {tracks.map((track) => {
          ...
          return <div key={track.id} className="rounded-xl border border-slate-200 p-4">
            <div className="flex items-start justify-between gap-3">
              <div><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Track</p><h4 className="mt-1 font-bold text-slate-950">Track #{track.id}</h4></div>
      ...
      </div> : <p className="py-8 text-center text-sm text-slate-500">Chưa có team nào được phân công.</p>}
    </MentorPanel>
  );
}
