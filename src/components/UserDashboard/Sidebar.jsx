...
export function Sidebar({ active, onNav }) {
  return (
    <aside className="fixed left-0 top-0 h-full w-64 flex flex-col z-30"
      style={{
        background: 'linear-gradient(to bottom, #F26F21 0%, #C2410C 45%, #7F1D1D 100%)',
      ...
      <nav className="flex-1 px-4 py-6 space-y-1">
        <p className="text-[10px] font-semibold tracking-widest uppercase px-3 mb-3"
          style={{ color: 'rgba(255,255,255,0.5)' }}>Navigation</p>
          ...
          return (
            ...
            >
              <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 transition-all duration-200"
                style={{
                  background: isActive ? 'rgba(194,65,12,0.12)' : 'rgba(255,255,255,0.15)',
                }}>
              ...
              <div>
                <p className="text-sm font-semibold transition-colors duration-200"
                  style={{ color: isActive ? '#C2410C' : '#FFFFFF' }}>{label}</p>
                <p className="text-[10px] transition-colors duration-200"
                  style={{ color: isActive ? 'rgba(194,65,12,0.7)' : 'rgba(255,255,255,0.55)' }}>{labelVi}</p>
              </div>
              {isActive && (
      ...
      {/* Footer */}
      <div className="px-4 pb-6 border-t pt-4 space-y-3"
        style={{ borderColor: 'rgba(255,255,255,0.15)' }}>
        {/* Avatar */}
        <div className="flex items-center gap-3 px-3 py-2 rounded-xl"
          style={{ background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.2)' }}>
          <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0"
            style={{ background: 'rgba(255,255,255,0.25)', color: '#FFFFFF', border: '1.5px solid rgba(255,255,255,0.5)' }}>
            N
          </div>
