  ...
  return (
    <div className="rounded-2xl p-6 transition-all duration-300"
        ...
        {[['file', 'File Upload'], ['repo', 'Repository']].map(([id, label]) => (
          <button key={id} onClick={() => setTab(id)}
            ...
            style={{
              ...
              boxShadow: tab === id ? '0 2px 8px rgba(242,111,33,0.15)' : 'none',
            }}>
            {label}
          </button>
        ))}
      ...
      {tab === 'file' ? (
        <div>
          ...
          >
            ...
            <CloudUpload className="w-8 h-8 mx-auto mb-2" style={{ color: dragging ? '#F26F21' : '#6b7280' }} />
            <p className="text-xs font-semibold text-[#111827]">
              {dragging ? 'Drop files here' : 'Drag & drop or click to upload'}
            </p>
            <p className="text-[10px] text-slate-500 mt-1">ZIP, PDF, or any project files</p>
          </div>
          {files.length > 0 && (
        ...
        <div className="mb-3">
          <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl mb-2 border border-slate-200 focus-within:border-[#F26F21] focus-within:ring-1 focus-within:ring-[#F26F21] transition-all duration-200"
          ...
          <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl border border-slate-200 focus-within:border-[#F26F21] focus-within:ring-1 focus-within:ring-[#F26F21] transition-all duration-200"
            style={{ background: '#FFFFFF' }}>
      ...
      <button
        ...
        style={{
          background: submitted ? '#D1FAE5' : 'linear-gradient(135deg, #F26F21, #c9520e)',
          color: submitted ? '#065F46' : '#fff',
          border: submitted ? '1px solid #A7F3D0' : 'none',
          boxShadow: submitted ? 'none' : '0 4px 14px rgba(242,111,33,0.2)',
        }}
        onMouseEnter={e => { if (!submitted) e.currentTarget.style.boxShadow = '0 6px 20px rgba(242,111,33,0.35)'; }}
        onMouseLeave={e => { if (!submitted) e.currentTarget.style.boxShadow = '0 4px 14px rgba(242,111,33,0.2)'; }}
      >
        {submitted ? (
