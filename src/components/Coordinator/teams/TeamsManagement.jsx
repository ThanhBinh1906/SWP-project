...
function Pagination({ page, totalPages, onPage }) {
  ...
  return (
    <div className="flex items-center justify-center gap-2 mt-4">
      <CoordinatorActionButton disabled={page === 1} onClick={() => onPage(page - 1)}>
        ← Prev
      </CoordinatorActionButton>
      <span className="text-sm text-slate-600 px-2">
        Trang {page} / {totalPages}
      </span>
      <CoordinatorActionButton disabled={page === totalPages} onClick={() => onPage(page + 1)}>
        Next →
      </CoordinatorActionButton>
    </div>
  );
...
export function TeamsManagement() {
  ...
  const fetchTeams = useCallback(async (p = 1) => {
    ...
    try {
      ...
      const res = await axiosInstance.get(`/admin/teams?${params.toString()}`);
      const data = res.data?.data;
      setTeams(data?.items || []);
      setTotalPages(data?.totalPages || 1);
      setPage(data?.pageNumber || 1);
    } catch (err) {
      setApiError(err?.response?.data?.message || "Không thể tải danh sách team.");
    } finally {
      setLoading(false);
    }
  ...
  const handleApprove = async (team) => {
    ...
    setActionError("");
    try {
      await axiosInstance.put(`/teams/${team.id}/approve`);
  ...
  const handleDisqualifyConfirm = async () => {
    ...
    try {
      ...
      setTeams(prev => prev.map(t => t.id === disqualifyTeam.id ? { ...t, status: "Disqualified" } : t));
  ...
  const handleAssignMentorConfirm = async () => {
    ...
    setActionError("");
    try {
      await axiosInstance.put(`/teams/${assignMentorTeam.id}/mentor`, { mentorId: selectedMentorId });
      const mentor = MOCK_MENTORS.find(m => m.id === selectedMentorId);
      setTeams(prev => prev.map(t =>
        t.id === assignMentorTeam.id ? { ...t, mentorId: selectedMentorId, mentorName: mentor?.name } : t
      ));
  ...
  const renderCell = (row, key) => {
    ...
    if (key === "mentor") return (
      <button
        ...
        onClick={() => { setAssignMentorTeam(row); setSelectedMentorId(""); setActionError(""); }}>
        {row.mentorId
          ? <span>{row.mentorName || `Mentor #${row.mentorId.slice(0, 8)}...`}</span>
    ...
    if (key === "status") return (
      <CoordinatorBadge tone={
        row.status === "Approved" ? "success"
        : row.status === "Disqualified" ? "danger"
        : "warning"
      }>
        {row.status}
      </CoordinatorBadge>
    ...
    if (key === "actions") return (
      <div className="flex gap-2 flex-wrap">
        <CoordinatorActionButton icon={icons.Eye}
          onClick={() => { setDetailTeam(row); setActionError(""); }}>
          Details
        </CoordinatorActionButton>
        {row.status === "Pending" && (
        ...
        {row.status === "Approved" && (
          <CoordinatorActionButton variant="danger"
            disabled={isActioning}
            onClick={() => { setDisqualifyTeam(row); setDisqualifyReason(""); setActionError(""); }}>
            Disqualify
          </CoordinatorActionButton>
        )}
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <CoordinatorPanel title="Team list"
        subtitle="Approve, inspect, or disqualify participating teams"
        icon={icons.UserRoundCog}>
        <FilterBar
          status={statusFilter} onStatus={handleStatusFilter}
          search={search} onSearch={setSearch}
        />

        {loading ? (
          <div className="flex items-center justify-center py-14 gap-2 text-sm text-slate-400">
            <Loader2 className="w-4 h-4 animate-spin" style={{ color: "#F26F21" }} />
            Đang tải...
          </div>
        ) : apiError ? (
          <div className="flex flex-col items-center py-10 gap-3">
            <p className="text-sm text-red-500">{apiError}</p>
            <CoordinatorActionButton onClick={() => fetchTeams(1)}>Thử lại</CoordinatorActionButton>
          </div>
        ) : filtered.length === 0 ? (
          <div className="py-14 text-center text-sm text-slate-400">Chưa có team nào.</div>
        ) : (
          <>
            <CoordinatorTable columns={columns} rows={filtered} renderCell={renderCell} />
      ...
      {/* Modal: Detail */}
      {detailTeam && (
        <ModalShell title={`Chi tiết: ${detailTeam.teamName}`} onClose={() => setDetailTeam(null)}
          actions={<CoordinatorActionButton variant="primary" onClick={() => setDetailTeam(null)}>Đóng</CoordinatorActionButton>}>
          <div className="space-y-2 text-sm">
            <InfoRow label="Tên team" value={detailTeam.teamName} />
            <InfoRow label="Trường" value={detailTeam.university} />
            <InfoRow label="Track" value={detailTeam.trackId ? `#${detailTeam.trackId}` : "—"} />
            <InfoRow label="GitHub" value={detailTeam.githubRepoLink} isLink={!!detailTeam.githubRepoLink} />
            <InfoRow label="Thành viên" value={`${detailTeam.members?.length ?? 0} người`} />
            <InfoRow label="Status" value={
              <CoordinatorBadge tone={detailTeam.status === "Approved" ? "success" : detailTeam.status === "Disqualified" ? "danger" : "warning"}>
                {detailTeam.status}
              </CoordinatorBadge>
            } />
          </div>
      ...
      {/* Modal: Disqualify */}
      {disqualifyTeam && (
        <ModalShell title={`Disqualify: ${disqualifyTeam.teamName}?`}
          onClose={() => { setDisqualifyTeam(null); setActionError(""); }}
          actions={
            <>
              <CoordinatorActionButton onClick={() => setDisqualifyTeam(null)} disabled={actionLoading === disqualifyTeam.id}>
                Huỷ
              </CoordinatorActionButton>
              <CoordinatorActionButton variant="danger"
                disabled={actionLoading === disqualifyTeam.id}
                onClick={handleDisqualifyConfirm}>
                {actionLoading === disqualifyTeam.id ? "Đang xử lý..." : "Xác nhận loại"}
              </CoordinatorActionButton>
            </>
          }>
          <div className="space-y-3">
            <p className="text-sm text-slate-600">
              Bạn có chắc muốn loại team <strong>{disqualifyTeam.teamName}</strong>?
            </p>
            <textarea className="min-h-24 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none"
              placeholder="Lý do loại team (tuỳ chọn)"
              value={disqualifyReason} onChange={e => setDisqualifyReason(e.target.value)} />
      ...
      {/* Modal: Assign Mentor */}
      {assignMentorTeam && (
        <ModalShell title={`Assign Mentor: ${assignMentorTeam.teamName}`}
          onClose={() => { setAssignMentorTeam(null); setActionError(""); }}
          actions={
            <>
              <CoordinatorActionButton onClick={() => setAssignMentorTeam(null)}>Huỷ</CoordinatorActionButton>
              <CoordinatorActionButton variant="primary"
                disabled={!selectedMentorId || actionLoading === assignMentorTeam.id}
                onClick={handleAssignMentorConfirm}>
                {actionLoading === assignMentorTeam.id ? "Đang xử lý..." : "Assign"}
              </CoordinatorActionButton>
            </>
          }>
          <div className="space-y-3">
            <p className="text-sm text-slate-500">
              Chọn mentor cho team <strong className="text-slate-800">{assignMentorTeam.teamName}</strong>:
            </p>
            ...
            <div className="space-y-2">
              {MOCK_MENTORS.map(m => (
                <button key={m.id} onClick={() => setSelectedMentorId(m.id)}
                  ...
                  style={{
                    background: selectedMentorId === m.id ? "rgba(242,111,33,0.08)" : "#F9FAFB",
                  ...
                  {selectedMentorId === m.id && (
                    <CheckCircle className="w-4 h-4 ml-auto" style={{ color: "#F26F21" }} />
                  )}
                </button>
              ))}
            </div>
    ...
    <div className="flex items-start justify-between gap-4">
      ...
      {isLink
        ? <a href={value} target="_blank" rel="noopener noreferrer"
            className="text-xs font-medium truncate" style={{ color: "#F26F21" }}>{value}</a>
