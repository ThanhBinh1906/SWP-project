...
export function TeamsManagement() {
  ...
  return (
    <div className="space-y-6">
        ...
        <CoordinatorTable
          ...
          renderCell={(row, key) => {
            ...
            if (key === "readiness")
              return (
                <div className="w-40">
                  <CoordinatorProgressBar value={row.readiness} />
                </div>
              );
            if (key === "actions")
              return (
      ...
      {selected && (
        ...
        >
          <div className="space-y-3 text-sm text-slate-600">
            <p>
              <b className="text-slate-900">Team:</b> {selected.name}
            </p>
            <p>
              <b className="text-slate-900">Submission:</b>{" "}
              {selected.submission}
            </p>
            <p>
              <b className="text-slate-900">Score:</b> {selected.score}
            </p>
            <CoordinatorProgressBar
      ...
      {disqualify && (
        <ModalShell
          title={`Disqualify ${disqualify.name}?`}
          onClose={() => setDisqualify(null)}
          actions={
            <>
              <CoordinatorActionButton onClick={() => setDisqualify(null)}>
                Cancel
              </CoordinatorActionButton>
              <CoordinatorActionButton
                variant="danger"
                onClick={() => setDisqualify(null)}
              >
                Confirm
              </CoordinatorActionButton>
            </>
